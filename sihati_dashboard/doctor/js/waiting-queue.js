...
async function loadWaitingQueue() {
  ...
  try {
    ...
    document
      ...
      ?.addEventListener("click", () => showAddToQueueModal(queuePatients));
  } catch (e) {
    pageError(e.message);
  }
  hidePageLoading();
...
async function callNextPatient() {
  try {
    const doctorId = api.getDoctorId();
    await api.request(`/waiting-queue/${doctorId}/next`, {
      method: "PUT",
    });
...
window.saveAddToQueue = async function () {
  ...
  try {
    ...
    closeModal();
    showToast(t("add_to_queue"), "success");
    await loadWaitingQueue();
  } catch (e) {
    showToast(e.message, "error");
  } finally {
