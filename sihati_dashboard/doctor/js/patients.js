...
function renderPatientCards(patients) {
  if (!patients.length)
    return `<div class="empty-state"><i class="fas fa-users"></i><p>${t("no_patients")}</p></div>`;
...
function bindPatientButtons() {
  document
    ...
    .forEach((btn) =>
      btn.addEventListener("click", () =>
        window.open(
          `patient-details.html?id=${btn.dataset.patientId}`,
          "_blank",
        ),
      ),
    );
  document
    .querySelectorAll(".btn-consult")
    .forEach((btn) =>
      btn.addEventListener("click", () =>
  ...
  document
    .querySelectorAll(".btn-prescription")
    .forEach((btn) =>
      btn.addEventListener("click", () =>
...
window.saveNewPatient = async function () {
  ...
  if (!data.fullName || !data.email || !data.phoneNumber || !data.password) {
    alertEl.innerHTML = `<div class="alert-error">${t("fill_all_fields")}</div>`;
    return;
  }
  if (data.password.length < 8) {
    alertEl.innerHTML = `<div class="alert-error">${t("pwd_min_length")}</div>`;
    return;
  }
  ...
  try {
    ...
    closeModal();
    showToast(t("patient_created") || "Patient créé", "success");
    await loadPatients();
  } catch (e) {
...
window.saveConsultation = async function (patientId) {
  ...
  try {
    ...
    closeModal();
    showToast(t("consultation_saved") || "Consultation enregistrée", "success");
    await loadPatients();
  } catch (e) {
    alertEl.innerHTML = `<div class="alert-error">${e.message}</div>`;
  } finally {
...
window.savePrescription = async function (patientId) {
  ...
  if (!valid || !medications.length) {
    alertEl.innerHTML = `<div class="alert-error">${t("add_medication") || "Ajoutez au moins un médicament"}</div>`;
    return;
  }
  const data = {
    patientId,
  ...
  try {
    ...
    closeModal();
    showToast(t("prescription_saved") || "Ordonnance enregistrée", "success");
    await loadPatients();
  } catch (e) {
    alertEl.innerHTML = `<div class="alert-error">${e.message}</div>`;
  } finally {
