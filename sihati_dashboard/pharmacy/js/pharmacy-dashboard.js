...
async function loadDashboard() {
  ...
  document.getElementById("pageContent").innerHTML = `
    ...
    <!-- Add Duty Toggle Card -->
    <div class="card">
      <div class="card-header">
        <h3><i class="fas fa-clock" style="color:var(--teal)"></i> ${pt("duty_status") || "Statut de garde"}</h3>
      </div>
      <div class="card-body">
        <div style="display:flex;align-items:center;justify-content:space-between;">
          <div>
            <p style="margin:0;"><strong>${pt("duty_tonight") || "Garde cette nuit"}:</strong> 
              <span id="dutyStatusText">${currentPharmacy?.isOnDutyTonight ? pt("yes") || "Oui" : pt("no") || "Non"}</span>
            </p>
            <p style="margin:4px 0 0;font-size:12px;color:var(--gray);">${pt("duty_description") || "Activez la garde pour apparaître dans les recherches des patients"}</p>
          </div>
          <button id="toggleDutyBtn" class="btn-primary" style="white-space:nowrap;">
            <i class="fas ${currentPharmacy?.isOnDutyTonight ? "fa-toggle-off" : "fa-toggle-on"}" style="margin-right:6px;"></i>
            ${currentPharmacy?.isOnDutyTonight ? pt("deactivate_duty") || "Désactiver la garde" : pt("activate_duty") || "Activer la garde"}
          </button>
        </div>
      </div>
    </div>
  ...
  const st =
    !item.inStock || qty === 0
      ...
      : qty < 10
        ...
        : pt("in_stock");
  const name =
    item.medication?.name || item.medication_name || item.medicationName || "-";
...
function bindEditButtons() {
  document.querySelectorAll(".btn-edit-stock").forEach((btn) => {
    btn.addEventListener("click", () =>
      showEditStockModal(btn.dataset.mid, btn.dataset.mname),
    );
  });
}
...
async function searchMedications(query) {
  ...
  try {
    ...
    if (r.success && r.data?.length) {
      res.innerHTML = `<div class="search-results-list">${r.data
        .map(
          (m) =>
...
async function toggleDuty() {
  ...
  try {
    ...
    currentPharmacy.isOnDutyTonight = newStatus;

    // Update duty badge in topbar
    const dutyBadge = document.getElementById("dutyBadge");
    if (dutyBadge) {
      if (newStatus) {
        dutyBadge.className = "duty-pill active";
        dutyBadge.innerHTML = `<span class="duty-dot"></span><span>${pt("on_duty") || "En garde"}</span>`;
      } else {
        dutyBadge.className = "duty-pill inactive";
        dutyBadge.innerHTML = `<span class="duty-dot"></span><span>${pt("off_duty") || "Hors garde"}</span>`;
      }
    }

    phShowToast(
      newStatus ? pt("duty_activated") : pt("duty_deactivated"),
      "success",
    );
    await loadDashboard(); // Refresh entire dashboard
  } catch (e) {
    phShowToast(e.message, "error");
  }
}
